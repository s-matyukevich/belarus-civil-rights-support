...
type User struct {
	...
	OAuthProvider   string `gorm:"not null;index"` // FB, VK, etc I plan to support only login through social networks.
	OAuthProviderId string `gorm:"not null;index"` // OAuth provider unique id. Because we never login with password we don't need to store password hash
